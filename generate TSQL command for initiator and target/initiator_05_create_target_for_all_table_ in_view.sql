...
DECLARE myCursor CURSOR FOR
	...
	WHERE TypDesc = 'USER_TABLE'
		--AND NOT EXISTS(SELECT 1 FROM ssbs.TargetObject AS mySSBS WHERE mySSBS.TableName = myTable.ObjectName)
	GROUP BY ObjectName;
OPEN myCursor;
...
BEGIN
    ...
    EXECUTE ssbs.dbasp_create_target_object @TableName = @myTableName, @ColumnName = @myColumnName, @IncludeClusteredIndex = @myIncludeClusteredIndex, @IncludeNonClusteredIndex = @myIncludeNonClusteredIndex;
